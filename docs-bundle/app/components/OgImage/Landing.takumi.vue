...
const { title, description, headline, socials = { icons: [], username: '' } } = defineProps<{
  title?: string,
  description?: string,
  headline?: string,
  socials?: {
...
<template>
  <div class="flex flex-col justify-center w-full h-full bg-blue-950 p-[100px]">
    <svg class="absolute right-0 top-0 fill-blue-200" height="593" viewBox="0 0 629 593" width="629"
      ...
      <defs>
        <filter id="filter0_f_199_94966" color-interpolation-filters="sRGB" filterUnits="userSpaceOnUse"
          height="1251.52" width="1255.25" x="0.873535" y="-659">
    ...
    <div class="flex flex-row gap-5">
      ...
      <div class="w-[340px] flex flex-col items-center justify-center gap-0 text-blue-300 dark:text-blue-900">
        ...
        <div class="flex items-center justify-center gap-4 mt-1">
          <UIcon class="w-12 h-12" mode="svg" name="i-tabler-brand-symfony" />
    ...
    <div v-if="socials.username || !socials.icons.length" class="flex flex-row justify-end items-center gap-4">
      ...
      <UIcon v-for="(icon, key) in socials.icons" :key="key" :name="icon" class="size-7 mx-1 text-white" mode="svg" />
    </div>
  </div>
</template>
